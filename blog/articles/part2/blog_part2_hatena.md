...
- 生成AIは、要約や翻訳、アイデア出し、文章作成など幅広い分野で活躍。実際の歯科医院での活用例（[院内掲示作成](https://chatgpt.com/share/683a4668-2d10-8008-8122-50556ef6c620)、[マニュアル作成](https://chatgpt.com/share/683a4904-0e18-8008-98e5-fc820bac89c8)、[論文検索](https://chatgpt.com/share/683a4bed-66e4-8008-863d-886cf483983c)など）を紹介しました。
- 「緻密で精密、ものすごい計算機」ではなく、**「人間らしい自然な文章を生み出す」**AI、それが **生成AI（ChatGPTやGeminiなど）** です。まさに秘書のような感覚で気軽に仕事や相談を頼めるレベルにまで進化しているのです。
- そして、AIを無料ではじめるなら**今がチャンス!!**。 各社赤字覚悟の無料枠を提供している今、使わない手はないっっっ！
...
ChatGPTは、この質問に対して、ナノ・オーラル合金は**架空の材料**なのにもかかわらず、あたかも実在するかのように、それも<span style="color: #ff5252">**自信満々**</span>に！スラスラとウソを並べ立てました。

この現象こそが、いわゆる**ハルシネーション**です。  
生成AIは、AIならではの便利さがある一方で、時に――いや、しばしば――存在しない情報を「自信満々に」作り上げてしまう、**困った作話癖**も持っています。

「論文を引用して答えて」と指示したら、存在しない論文をでっち上げられたコワイ――なんて経験がある方もいるのではないでしょうか。

実際、そんな“ウソ引用”が、重大な結果を招いたケースもあります。2023年アメリカで、弁護士が裁判所に提出した書類にChatGPTで生成した6件の**架空の判例**が含まれており、**5,000ドルの罰金**を科されるという事件まで起きています。[^1]

間違いの許されない法律や医療の分野では、AIの情報を鵜呑みにするのはとても危険なこと。使用者自身が**必ず裏取り**を行う必要があります。  
前回のテーマ**今がチャンス！** に続く今回のテーマは…
...
生成AIってどのように作られて、どのように動いているのでしょう、その仕組み理解して、なぜハルシネーションが起きるのかを理解しましょう
機械学習のモデルについて。モデルという言葉は使わず、線形回帰の例を
前回の記事より
> **生成AI**が学習したのは、人間が読むのに**約6300年**かかるとも言われる、**膨大な量の文章**です。その文章から**言語を紡ぐ力**を学び取りました。
何度も何度も次の単語を予測している
そうしてできあがったAI
(ファインチューニングで要約や翻訳、質問への回答などのタスクに対応できるようにする)
RLHFで躾する
この仕組みそれぞれに落とし穴がある

#### 原因：事前学習
AI(モデル)はコーパスを学習して作られる
コーパスに含まれないものは「知らない」　非公開情報、マイナーな情報、最新情報など
コーパスが間違っていることもある…ブログ記事など
事実性ハルシネーションが起きる原因はここに

#### 原因：強化学習
RLHFにより受け答えを学習する
sycophancyしてしまったりする
忠実性ハルシネーションを減らすためのステップだが、逆にこの仮定で生まれるハルシネーションもある

#### 原因：出力
確率分布に従って次単語を出力しているだけ
確率が高いものだけを選んで出力すると変な文章になる
確率が低いものも選ぶことで人間の文章に近づく
その確率はコーパスから得られたものなので、因果に従っているわけではなく、事実かどうかではなく、もっともらしいかで判定されている

> ##### コラム
> なんで統計学的予測というふわふわした方法をとるの？
> 莫大な選択肢がある　n-gramの説明、bigramですら日本語の語彙で考えるとたくさんになる
> CPUは1秒間に~回計算ができる
> 次元の呪い組み合わせを考えると宇宙が爆発しても計算時間が足りない
> なので、今のところ予測モデルを作るという方法がうまく行っている

#### 原因：推論
reasoningモデルについて説明
メリットもたくさんあるが、推論の過程で間違えると
雪だるま式に間違いが増える

### 対策
「事実かどうか」を判断させるのはだめ
使用者が既に知っている情報について答えてもらう（歯科ブログなど）
与えた情報の中に含まれるかどうかを判断させる
...
AIに検索させる
例えば質問したあとにその根拠となる記事を探して。と言うと記事を探してきてくれる
その記事が合っているかどうかもわからないが、少なくとも検証に使うための根拠が１つ増える
最初から検索するのとは違う、検索ワードも内容からAIが考えてくれる
> ##### コラム
> 辞書からgoogleからwikiからそんな感じで調べ物は変遷している
> 最良の方法はない、常に疑い続ける、疑問を持ち続け、調べる力を養うしかない

> ##### コラム2
> 文字数を数える 計算する
> 一見AIなら得意そうにみえる、もしくは人間にも簡単にできることでも、
> これらはLLMと相性が悪い　なぜなら言葉をトークンという単位で扱っているので
> トークンには文字数の情報が含まれていないか、含まれていても曖昧なので、齟齬が出てしまう
> これらのタスクを得意とするのはプログラミング　LLMがfunction callingでプログラミングしてくれるならこれらは簡単に解決できる

#### RAG
資料を渡し、その中から答えてもらう仕組み
##### notebookLM
リンク貼る、使い方説明の記事を探す

#### 不確実性の判定
何度か出力させて結果がブレるかどうかを見る




### 文章を作るしくみ
前回の記事で生成AIは**人が読み終えるのに数千年かかる**といわれる程の圧倒的な量の文章[^2]を学習していると述べました。
...
「試合に負けて泣きながら食べたグミは少し…」
“グミ”自体は本当は甘いのに、文脈を読むと“しょっぱい”と理解されている。

	•	これができるのは「セルフアテンション」という仕組みのおかげ。
	•	文中の単語同士の関係性をみることができる
...
次回予告
ハルシネーションと対策はわかったけど実際どうやって使うのさ？というあなたへ
